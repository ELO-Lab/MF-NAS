...
def get_score(net, x, target, device, split_data, measure='meco'):
    result_list = []
...
@measure('meco_opt', bn=True)
def compute_meco_opt(net, inputs, targets, split_data=1, loss_fn=None):
    device = inputs.device
    # Compute gradients (but don't apply them)
    net.zero_grad()

    try:
        meco_opt = get_score(net, inputs, targets, device, split_data=split_data, measure='meco_opt')
    except Exception as e:
        print(e)
        meco_opt = np.nan, None
    return meco_opt