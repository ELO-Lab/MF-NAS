...
def train(model, data_loader, criterion, optimizer):
    ...
    for step, (inputs, targets) in enumerate(tqdm(data_loader)):
...
def inference(model, data_loader, criterion):
    objs = AverageMeter()
    top1 = AverageMeter()
    model.eval()
...
def run(kwargs):
    ...
    model = SEARCH_SPACE.get_model(genotype, search=False, device=device)
    model.to(device)
    ...
    train_status = {'score': best_score, 'seed': kwargs.seed}
